				...
				<div class="join_form">
					<table>
						...
						<tbody>
							<tr>
								<th><span>아이디</span></th>
								<td><input type="text" id="id"name="id" placeholder="영문자+숫자만 입력 가능"> <input type="hidden" name="ID_Duplication"
						value="ID_Uncheck" > <font id="msg" style="font-size: 0.8em;"> &nbsp; <!--중복확인 문구 --></font></td>
							</tr>
							<tr>
								<th><span>이름2</span></th>
						...
						onkeyup="moveNext(this,4,document.Join_form.email)"></td>
							...
							<tr class="email">
								<th><span>이메일</span></th>
								<td><input type="text" class="email" placeholder="이메일 입력" name="email" id="email">
...
$(document).ready(function() {  //이메일 유효성 
    ...
    $("#email").focusout(function() {
        ...
        if (email.length <= 2 || email.length > 16){
        	...
        	return false; 
         	...
         	$('#emailCheck').html('<span style="color:red;"> </span>');
        }
  ...
  /*  $.ajax({   //이메일 아작스실패 ( '@' 포함되서 디비에 입력되는데, 아작스검사할때 스플릿해서 나누기 까지함.)
            ...
            success : function(result) {
            	...
            	$("#emailCheck").html("사용 할 수 있는 이메일 입니다.  <button type='button' id='textBtn' class='id_btn' onclick='idbtn()'> 사용 </button>");
