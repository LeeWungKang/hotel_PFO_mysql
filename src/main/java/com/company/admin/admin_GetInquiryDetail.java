...
@WebServlet("/admin_GetInquiryDetail")
public class admin_GetInquiryDetail extends HttpServlet {
	private static final long serialVersionUID = 1L;
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		System.out.println("admin_GetInquiryDetail --  관리자 고객응대 페이징");
		...
		try {
			...
			if (rs.next()) {
			  ...
			  // 댓글목록을 화면에 넣기위해 위에서 사용한 자원은 잠시 닫아주고 재활용하면서 새로운 sql문 시작 // 날짜+ 시간까지 추가하는 sql
			  sql =
			  ...
			  ArrayList<InquiryReVo> iqList = new ArrayList<InquiryReVo>(); 
			  while(rs.next()) { InquiryReVo iqvo = new InquiryReVo();
			...
			RequestDispatcher dis = request.getRequestDispatcher("index.jsp?filePath=./admin/getInquiry");
